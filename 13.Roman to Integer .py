...
# Однако цифра, обозначающая четыре, не является IIII. Вместо этого цифра четыре записывается как IV.
...
def romanToInt(s: str) -> int:
    ...
    for i in s:
        result.append(meanings[i])
        print(result)
    for index, num in enumerate(result):
        if index == len(result) -1:
            answer += num
            return answer
        elif num >= result[index + 1]:
            answer += num
        else:
            new_num = result[index + 1] - num
            answer += new_num
